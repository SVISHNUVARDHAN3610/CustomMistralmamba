...
class FlushingStreamHandler(logging.StreamHandler):
    """StreamHandler that flushes immediately to prevent interactive notebook buffer stalls."""
...
def setup_logging(run_dir: Path, *, level: int = logging.INFO) -> logging.Logger:
    ...
    logger = logging.getLogger("kaggle_tpu_smpd_train")
...
def build_training_config(vocab_size: int, seq_len: int = 1024) -> HybridMambaMoEConfig:
    ...
    stream_chunked_ce_loss is set to False with memory_chunk_size=seq_len to compile
    a single static, highly fused cross entropy graph on TPU.
    """
    hidden_size = 768
    num_heads = 12
    head_dim = 64
...
def init_tpu_spmd(logger: logging.Logger) -> tuple[torch.device, int]:
    ...
    if not XLA_AVAILABLE or xr is None or xm is None:
        raise RuntimeError(
            ...
            "pre-training/kaggle_tpu_smpd_train.py must be executed in a TPU environment "
    ...
    def _monitor(self) -> None:
        ...
        while not self._stop.wait(poll_seconds):
            ...
            with self._lock:
                ...
                phase = self._phase
                if step <= 0:
                    step0_heartbeat_interval = 30.0
                    should_heartbeat = (
                        active
                        and elapsed >= step0_heartbeat_interval
                        and (
                            self._last_warning == 0.0
                            or since_warning >= step0_heartbeat_interval
                        )
                    )
                    if should_heartbeat:
                        self._last_warning = now
                        self.logger.info(
                            "TPU v5e-8 compilation heartbeat | phase=%s | elapsed: %.1fs | "
                            "Tracing & fusing XLA HLO graphs across 8 cores. Please wait...",
                            phase,
                            elapsed,
                        )
                else:
                    ...
                    should_warn = (
                        ...
                        and (
                            self._last_warning == 0.0
                            or since_warning >= effective_timeout
                        )
                    )
                    if should_warn:
                        self._last_warning = now
...
def train(args: argparse.Namespace, logger: logging.Logger) -> None:
    ...
    cfg = build_training_config(vocab_size=args.vocab_size, seq_len=args.seq_len)
    ...
    # 3. Construct Model on CPU and Calibrate SSM Norm Thresholds
    logger.info("Constructing HybridForCausalLM model on CPU...")
    model = HybridForCausalLM(cfg)
    logger.info(
        "Calibrating SSM norm thresholds on CPU before XLA device placement (avoids XLA quantile compilation)..."
    )
    model.model.calibrate_ssm_norm_thresholds()
    logger.info("SSM calibration complete. Moving model to XLA device %s...", device)
    ...
    try:
        ...
        while global_step < args.max_steps:
            ...
            while True:
                ...
                # 7. Single D2H Host Transfer Flush (every step for early warmup <10, then every log_interval)
                if global_step < 10 or global_step % args.log_interval == 0:
                    assert (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="DataLoader worker processes for shard reading (default: 0 runs in-process "
        "with mmap, eliminating Python fork IPC deadlocks in Kaggle interactive notebooks).",
    )
