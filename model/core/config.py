...
class HybridMambaMoEConfig(MixtralConfig):
    ...
    # retrieval error on L2-normalized vectors (err in [0, 4]). The old
    # 1.2e-4 value assumed an unnormalized .sum(dim=-1) over hidden_size=512
    # dimensions — 0.0614 = 1.2e-4 * 512 preserves the original effective
    # weighting under the new normalized/mean formulation.
    lambda_assoc: float = 0.0614
    assoc_warmup_fraction: float = 0.05
    assoc_sample_count: int = 24
    # Hard backstop clamping per-sample retrieval error in associative_retrieval_loss
    # (normalized err is already ≤ 4; this guards against float edge cases).
    assoc_err_clip: float = 25.0
    # Hinge penalty (see assoc_state_norm_loss) keeping the post-write memory
